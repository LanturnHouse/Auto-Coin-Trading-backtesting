...
print("data loading")
df = pyupbit.get_ohlcv(coin_type,interval = 'minute5' ,count = 10100)
df = df.reset_index()
print("done!\n\n\n")
...
for i in range(2,len(df['index'])):
    ...
    if (tick == 0 and ((df.at[i-1,'close'] - df.at[i,'close'] >= tick_value and df.at[i-1,'open'] - df.at[i-1,'close'] >= tick_value) or
