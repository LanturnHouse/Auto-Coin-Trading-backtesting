...
#         circulation = False
        ...
        if ll_1[0] <= ll_1[1] <= ll_1[2] > ll_2[0] > ll_2[1] > ll_2[2]:
            if ll_2[0] - ll_2[2] > tick_value:
                ...
                if cut_sell == False:
                    ...
                    for k in range(0,len(coin_buy_list)):
                        # 판매
                        cell_revenue[k] = df['close'][i] * coin_buy_num[k]
                        sell_bool_ += f'cut sell!{k}'
    ...
    #     else:
    #         if ll_1[0] < ll_1[1] < ll_1[2]:
    #             if ll_1[0] - ll_2[2] < tick_value * 4:
    #                 for k in range(0,len(coin_buy_list)):
    #                     if coin_buy_list[k] != 0:
    #                         # 판매
    #                         cell_revenue[k] = df['close'][i] * coin_buy_num[k]
    #                         sell_bool_ += f'cut sell!{k}'
...
df[['index', 'close', 'tick', 'tick_num', 'avg_value', 'buy_bool' ,'sell_bool','sell_price','limit','ror','buy_coin_num','cell_revenue','all_cell_revenue', "buy_count", "sell_count", "cut_sell_num"]].to_excel('ACT_backtesting.xlsx')
